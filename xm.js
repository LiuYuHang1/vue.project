...
async function loadData() {
  ...
  for (var i = 0; i < savedCategories.length; i++) {
    ...
    //     axios
    //       .post(
    //         "https://www.xiaomiyoupin.com/api/gateway/detail",
    //         {
    //           groupName: "details",
    ...
    goods.data.data.data.goods.forEach((item, index) => {
      ...
      p.content = "小米-" + item.data.goodsInfo.gid;
  ...
  for (let i = 0; i < products.length; i++) {
    const data = await axios
      .post(
        ...
        {
          groupName: "details",
          groupParams: [[products[i].content.replace("小米-", "") + ""]]
        },
        {
          timeout: 5000
        }
      ...
      .catch((err) => console.log(err));
    let strContent = "";
    if (data.data.data.goods.goodsInfo.introExt) {
      data.data.data.goods.goodsInfo.introExt[0].imagesArray.forEach((pimg) => {
        strContent += `<p><img src="${pimg}"/></p>`;
      });
    }
